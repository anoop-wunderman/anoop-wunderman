...
import { produceCRMData } from './server.kafka';

import { socketRun } from './kafka';

import { getCollection } from './server.mongo';

import { getDownloadLink, getPDFs } from './server.aws';
...
export function app() {
  ...
  // Run scheduler and produce CRM data
  server.get('/api/ish/produce', produceCRMData);

  // Fetch Master template data
  server.get('/api/ish/mastertemplate', (_req, res) => {
    getCollection('mastertemplate', (data: any) => {
      res.status(200).json(data);
    });
  });

  // Fetch Last Offset
  server.get('/api/ish/lastoffset', (_req, res) => {
    getCollection('sensors', (data: any) => {
      res.status(200).json(data);
    });
  });

  // Fetch Last Operating Cost
  server.get('/api/ish/operatingcost', (_req, res) => {
    var arr = (_req.query['devices'] as string).split(',');
    getCollection(
      'operatingcost-dedup',
      (data: any) => {
        res.status(200).json(data);
  ...
  // Fetch Auto Pilot Savings
  server.get('/api/ish/autopilotsavings', (_req, res) => {
    var arr = (_req.query['devices'] as string).split(',');
    getCollection(
      'autopilotsavings-dedup',
      (data: any) => {
        res.status(200).json(data);
      },
      'au',
  ...
  server.get('/api/ish/opreportslist', (_req, res) => {
    const start = _req.query['sdate'] as string;
    const end = _req.query['edate'] as string;
    const machineIds = _req.query['ids'] as string;
    getPDFs(machineIds, start, end, data => {
      res.status(200).json(data);
    });
  });

  // download link
  server.get('/api/ish/opreport', (_req, res) => {
    const key = _req.query['key'] as string;
    getDownloadLink(key, data => {
      res.status(200).json(data);
    });
  });

  // Fetch Auto Pilot Savings
  server.get('/api/ish/fetchnotification', (_req, res) => {
    var arr = (_req.query['devices'] as string).split(',');
    getCollection(
      'notification',
      (data: any) => {
